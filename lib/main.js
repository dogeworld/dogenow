...
(async () => {
    ...
    let gopaxDisplay;
    let binanceDisplay;
    skynode_1.el(".content", skynode_1.el("h1", "도지나우 - 도지코인 현재가격 모든 거래소 정보"), upbitDisplay = skynode_1.el(".price-display"), gopaxDisplay = skynode_1.el(".price-display"), binanceDisplay = skynode_1.el(".price-display"), skynode_1.el("a.madeby-button", { href: "https://dogeworld.dog" }, skynode_1.el("img", { src: "/images/madeby.png", width: "300" }), skynode_1.el("img", { src: "/images/bottom.png", width: "300" }))).appendTo(skynode_1.BodyNode);
    const refresh = async () => {
