...
(async () => {
    ...
    let huobiDisplay;
    let binanceDisplay;
    skynode_1.el(".content", skynode_1.el("h1", "도지나우 - 도지코인 현재가격 모든 거래소 정보"), upbitDisplay = skynode_1.el(".price-display", skynode_1.el("span.exchange", "업비트")), gopaxDisplay = skynode_1.el(".price-display", skynode_1.el("span.exchange", "고팍스")), huobiDisplay = skynode_1.el(".price-display", skynode_1.el("span.exchange", "후오비 코리아")), binanceDisplay = skynode_1.el(".price-display", skynode_1.el("span.exchange", "Binance")), skynode_1.el("a.madeby-button", { href: "https://dogeworld.dog" }, skynode_1.el("img", { src: "/images/madeby.png", width: "300" }), skynode_1.el("img", { src: "/images/bottom.png", width: "300" }))).appendTo(skynode_1.BodyNode);
    const refresh = () => {
        (async () => {
        ...
        (async () => {
            const huobiData = await superagent_1.default.get("https://api-cloud.huobi.co.kr/market/history/trade?symbol=dogekrw");
            huobiDisplay.empty().append(skynode_1.el("span.exchange", "후오비 코리아"), skynode_1.el("span.price", `${huobiData.body.data[0].data[0].price} 원`));
        })();
        (async () => {
