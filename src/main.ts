...
(async () => {
    ...
    el(".content",
        ...
        huobiDisplay = el(".price-display", el("span.exchange", "후오비 코리아")),
    ...
    const refresh = () => {
        ...
        (async () => {
            const huobiData = await superagent.get("https://api-cloud.huobi.co.kr/market/history/trade?symbol=dogekrw");
            huobiDisplay.empty().append(el("span.exchange", "후오비 코리아"), el("span.price", `${huobiData.body.data[0].data[0].price} 원`));
        })();
        (async () => {
